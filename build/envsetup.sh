...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        ZIPFILE=darkrom-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
if [ -d $(gettop)/prebuilts/clang/linux-x86/host/sdclang-3.8 ]; then
    case `uname -s` in
        Darwin)
        ...
        *)
            export SDCLANG=true
            export SDCLANG_PATH=$(gettop)/prebuilts/clang/linux-x86/host/sdclang-3.8/bin
